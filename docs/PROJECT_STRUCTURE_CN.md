...
| `tools/verify_protected_lane_review_task.py` | protected-lane 审核表交给人工前的验证器。检查必需列、重复 provider/reason、Amazon `provider_detail_url` 保留、manual 列是否为空、summary 计数是否一致，以及 XLSX 是否有可点击链接公式。 |
