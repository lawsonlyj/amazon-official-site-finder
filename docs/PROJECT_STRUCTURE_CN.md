...
| `tools/run_calibration_followup.py` | 人工填完 label-gap/sample XLSX 后的续跑入口。读取上一轮 `calibration_cycle_summary.json`，复用原输入、pattern-release、policy 和采样参数，合并新的 `--filled-sample`，重跑校准并写出 `calibration_followup_decision.json/md`，明确当前 gate 是否允许 A 应用阈值、lane 或 pattern-release 改动。 |
