...
| `tools/run_calibration_cycle.py` | 一键生成下一轮校准材料：recall/precision 证据组合报告、均衡 pattern-validation 审核表、空表评估和 cycle summary；也可用 `--filled-sample` 读取填好的样本并汇总 pattern 采纳/拒绝建议。 |
